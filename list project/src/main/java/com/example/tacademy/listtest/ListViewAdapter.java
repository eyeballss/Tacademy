...
public class ListViewAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        final ImageView poster = view.findViewById(R.id.ImageView_starIcon);
        ...
        comment.setText(data[i][0]);

        if(thumbnails==null) return view;

        title.setText(Html.fromHtml(String.valueOf(Html.fromHtml(thumbnails.get(i).title))));
        comment.setText(thumbnails.get(i).pubDate+" "+thumbnails.get(i).cpname);

//        imageLoader.displayImage("http://13.59.72.237:3000/images/"+(i-1)+".png", poster, new ImageLoadingListener(){
        imageLoader.displayImage(thumbnails.get(i).thumbnail, poster, new ImageLoadingListener(){
            ...
            public void onLoadingFailed(String imageUri, View view, FailReason failReason) { //로딩이 실패함.
                poster.setImageDrawable(null); //url 에서 못 가져오면 사진 자체를 비워둠.
            }
